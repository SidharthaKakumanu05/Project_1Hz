...
def build_connectivity(cfg):
    """
    Build all the connectivity matrices for the network.

    Returns a dictionary 'graph' where each key corresponds
    to a projection (e.g. "PF_to_PKJ") and contains arrays
    of presynaptic indices, postsynaptic indices, and possibly weights.
    """
    ...
    # -----------------------------
    # Each PKJ receives inputs from ~N_PF_POOL/4 PFs
    ...
    # -----------------------------
    # Each BC gets inputs from ~N_PF_POOL/8 PFs
    ...
    # -----------------------------
    # Each CF targets one PKJ, spread evenly across population
    cf_pre = np.arange(N_CF, dtype=np.int32)
    cf_post = np.linspace(0, N_PKJ - 1, N_CF, dtype=np.int32)  # distributes CFs across PKJs
    graph["CF_to_PKJ"] = {
    ...
    # -----------------------------
    # Each PKJ receives inhibition from ~N_BC/4 basket cells
    bc_conn = N_BC // 4
    ...
    graph["BC_to_PKJ"] = {
        ...
        "g": bc_g,   # inhibitory conductances
    }

    # -----------------------------
    # PKJ → DCN (random sparse inhibition)
    # -----------------------------
    # Each DCN neuron gets input from ~N_PKJ/8 PKJs
    pkj_conn = N_PKJ // 8
    ...
    # MF → DCN (mossy fibers)
    # -----------------------------
    # For now, one mossy fiber per DCN neuron (1:1 mapping)
    # Each pre/post pair index matches exactly
    graph["MF_to_DCN"] = {
